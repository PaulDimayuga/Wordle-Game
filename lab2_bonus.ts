...
function Search_URL(){
    ...
    if (searchBox !== null) {
        const words_URL = (searchBox as HTMLInputElement).value;
        if (words_URL == ''){
...
function Game_page(){
    ...
    game_div?.classList.add("game_Proper");
...
function create_Grid(){
    ...
    brd_container?.appendChild(brd);
    game_div?.appendChild(brd_container);
}
...
function create_Keyboard(){
    ...
    kb?.appendChild(kb_row1_e);
    kb?.appendChild(kb_row2_e);
    kb?.appendChild(kb_row3_e);
    game_div?.appendChild(kb);
}


function get_Keyboard(k:any){
...
function update_Grid(ltr:string){
    
    if (box_num < 5){
        let box = document?.getElementById(`grid_row${row_num}_box${box_num}`);
        box?.setAttribute("state", "guess");
        box!.textContent = ltr;
        box_num++;
    }
...
function Enter_press(k:any){
    ...
    if (k.key === "Enter"){
        let grid_input = "";
        for (var i = 0; i < 5; i++){
            let box = document?.getElementById(`grid_row${row_num}_box${i}`)
            grid_input += box?.textContent;
        }
...
//  Checkpoint 3 Start
function Checker(guess:string, ans:string){
    let ans_idx: {[ltr:string] : number[]} = {};
    let corr_idx: {[ltr:string] : number[]} = {};
    let misp_idx: {[ltr:string] : number[]} = {};
    for (var i = 0; i < 5; i++){
    ...
    for (var i = 0; i < 5; i++){
        ...
        if (guess.charAt(i) in corr_idx) {
            if (corr_idx[guess.charAt(i)].includes(i)){
                box?.setAttribute("state", "correct");
                key?.setAttribute("state", "correct");
            } else if (guess.charAt(i) in misp_idx){
                if (misp_idx[guess.charAt(i)].includes(i)) {
                    box?.setAttribute("state", "misplaced");
                    if (key?.getAttribute("state") != "correct"){
                        key?.setAttribute("state", "misplaced");
                    }
                }
            } else {
                box?.setAttribute("state", "wrong");
                if (!(key?.getAttribute("state") == "correct" || key?.getAttribute("state") == "misplaced")){
                    key?.setAttribute("state", "wrong");
                }
            }
        }
        else if (guess.charAt(i) in misp_idx){
...
(searchBox as HTMLInputElement).value = def_url;
let searchBtn = document.getElementById("Search-URL-button");
searchBtn?.addEventListener("click", Search_URL);